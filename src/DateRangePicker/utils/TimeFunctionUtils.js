...
import { ModeEnum } from './../DateRangeContainer';
...
const workOutMonthYear = (date, secondDate, mode) => {
...
export const startDateStyle = () => {
	return [' is-within-range is-from', ' is-active'];
};

export const endDateStyle = () => {
	return [' is-within-range is-to', ' is-active'];
};
...
export const hoverCellStyle = (between) => {
	return (between) ? ([' is-within-range', ' is-active']) : (['',' is-active']);
};
...
export const invalidStyle = (future) => {
	return (future) ? ([' text-grey', null]) : ([' text-grey', '']);
};
